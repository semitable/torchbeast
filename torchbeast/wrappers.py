...
class TorchBeastEnv(gym.Wrapper):

    def __init__(self, env):
        super().__init__(env)
        self._prev_rewards = np.zeros(len(self.observation_space), dtype=np.float32)
    
    def reset(self):
        obs = super().reset()
        return {"obs": np.stack(obs), "rew": self._prev_rewards}

    def step(self, action):
        obs, reward, done, info = self.env.step(action)
        self._prev_rewards = np.array(reward, dtype=np.float32)
        return {"obs": np.stack(obs), "rew": np.array(reward)}, sum(reward), all(done), info

class TorchBeastObservation(ObservationWrapper):

    def observation(self, obs):
        return np.stack(obs)

class TorchBeastReward(RewardWrapper):
